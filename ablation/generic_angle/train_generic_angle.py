...
def collate(items):
    ...
    # Generic-angle ablation: retain angle magnitudes, but collapse every
    # triplet chemical-type code to one shared category.
    x=[{**g,'triplet_type':torch.zeros_like(g['triplet_type'])} for g in x]
    r=[{**g,'triplet_type':torch.zeros_like(g['triplet_type'])} for g in r]
...
def train_seed(saved,train,val,seed,device,cfg):
    ...
    for epoch in range(1,cfg['dual_epochs']+1):
        ...
        common.event('generic_angle_epoch',seed=seed,epoch=epoch,loss=float(np.mean(losses)),delta_mae=m['delta_mae'],
    ...
    saved={**saved,'architecture':'dual_cubic_delta_generic_angle_v1','model_state_dict':best,'best_epoch':best_epoch,
...
def export_readable_results():
    ...
    out=OUT;db_path=ROOT/'transfer_learning/results.sqlite'
    with sqlite3.connect(db_path) as db:
        epochs=[json.loads(row[0]) for row in db.execute(
            "select payload from events where kind='generic_angle_epoch' order by time")]
        def latest(kind):
    ...
    bundle=torch.load(out/'generic_angle_models.pt',map_location='cpu',weights_only=False)
    ...
    lines += ['', '## 文件说明','',
      '- `generic_angle_models.pt`：Generic-angle 五个训练模型，种子为 42、123、2026、3407、7777。',
...
def main():
    ...
    train,val,test=PairSet(train,cubic,proxy,raw),PairSet(val,cubic,proxy,raw),PairSet(test,cubic,proxy,raw)
    base=torch.load(ROOT/'transfer_learning/base_models.pt',map_location='cpu',weights_only=False);path=OUT/'generic_angle_models.pt'
    ...
    matrices=[];deltas=[];truth=None;dtruth=None;test_matrices=[];test_deltas=[];test_truth=None;test_dtruth=None
    for seed in cfg['seeds']:
        ...
        tm=evaluate(model,test,device,base[seed]['target_mean'],base[seed]['target_std'],cfg['dual_batch_size'])
        test_matrices.append(tm['pred']);test_deltas.append(tm['delta_pred']);test_truth=tm['truth'];test_dtruth=tm['delta_true']
    matrix=np.asarray(matrices);dm=np.asarray(deltas);tmat=np.asarray(test_matrices);tdm=np.asarray(test_deltas)
    result={'absolute_mae':float(abs(matrix.mean(0)-truth).mean()),
      'delta_mae':float(abs(dm.mean(0)-dtruth).mean()),'mean_uncertainty':float(dm.std(0,ddof=1).mean()),'seeds':cfg['seeds'],
      'test_absolute_mae':float(abs(tmat.mean(0)-test_truth).mean()),
      'test_delta_mae':float(abs(tdm.mean(0)-test_dtruth).mean()),
      'test_mean_uncertainty':float(tdm.std(0,ddof=1).mean()),
      'train_pairs':len(train),'validation_pairs':len(val),'test_pairs':len(test)}
    (OUT/'generic_angle_results.json').write_text(json.dumps(result,indent=2),encoding='utf-8')
    common.save_atomic(bundle,OUT/'generic_angle_models.pt');common.event('generic_angle_complete',**result)
