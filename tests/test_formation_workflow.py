...
sys.path[:0]=[str(ROOT/'step5'),str(ROOT/'step6')]
import step5
import step6
...
def test_proxy_dataset_does_not_mutate_original_graph():
    ...
    ds=step5.ProxyDataset([item],{'a':-2.5})
...
def test_pair_error_is_offset_invariant_and_composition_balanced():
    ...
    m=step6.summarize(items,labels,matrix)
...
def test_acquisition_unique_deterministic_budget():
    ...
    for strategy in ('active','random'):
        first=step6.acquisition(items,['0'],matrix,5,strategy,19)
        assert len(first)==len(set(first))==5 and '0' in first
        assert first==step6.acquisition(items,['0'],matrix,5,strategy,19)
